...
public class RendererAI {
		...
		World world;
		ControllerAI controller;
		Texture clickTt;
		String randomQuote[] = {"www.jangkoo.com",
		...
		Label souls,hp,dmg,gold,rank;
		public RendererAI (ShadowFiend shadowfiend) {
		...
		public void render ( ControllerAI controller,float delta) {
			...
			spriteAI.setPosition((float)controller.heroAI.v2Position.x-75 , (float)controller.heroAI.v2Position.y -75);
			spriteAI.setRotation((float) (controller.heroAI.getAngle()));
			sr.setProjectionMatrix(controller.camera.combined);
			sr.begin(ShapeType.Filled);
			sr.setColor(new Color(0,0,1,0));
			sr.rect((float)controller.heroAI.v2Position.x-75,(float)controller.heroAI.v2Position.y +75, 150*controller.heroAI.getHp()/200, 10);
			sr.end();
			spriteBatch.begin();
			...
			if(controller.gameState == GameState.Lost)
				spriteBatch.draw(new Texture("data/game_over.png"),Settings.CAMERA_WIDTH/2 - 300,Gdx.graphics.getHeight()/2 - 100);
			if(controller.gameState == GameState.Won)
				spriteBatch.draw(new Texture("data/victory.png"),Settings.CAMERA_WIDTH/2 - 300,Gdx.graphics.getHeight()/2 - 100);
			if(controller.click.aliveTime <Raze.RAZE_LIVE_TIME)
				spriteBatch.draw(clickTt,controller.click.v2Position.x,controller.click.v2Position.y,70,70);
